...
public class testCqlDriver {
    public static void main(String[] args) throws IOException,CQLException,InterruptedException,ExecutionException {
        ...
        String statusURL = "http://116.62.141.205:8080/gplatform_boot/rest/api/task/executeCallback";
        ...
        stormList.HanldRuleContextThread();
        //通过子线程获取输入输出量；
        /*
        SendThroughout sendThroughout = new SendThroughout(stormList.getTaskId(),stormList.getExecuteResult());
        FutureTask<ExecuteResult> task = new FutureTask<>(sendThroughout);
        Thread t = new Thread(task);
        t.start();
        stormList.getExecuteResult().setInputCount(task.get().getInputCount());
        stormList.getExecuteResult().setInputCount(task.get().getOutputCount());
        */
        while(true){
            Thread.sleep(30000);
            ...
            */
            ...
            try{
                HttpRequest.httpPostWithJson(statusURL, jsonString);
            }catch (Exception e){
                e.printStackTrace();
            }

...
import org.jn.streamcql.userdefined.datasource.ipfilter.IPFilter;
...
public class Operator implements IFunctionStreamOperator{
        ...
        Thread contextThread = new Thread(new Runnable() {
            ...
            public void run() {
                while (true){
                    try{
                        Thread.sleep(10000);
                        inWordsCounter = IPFilter.getInWordsCounter();
                        outWordsCounter = IPFilter.getOutWordsCounter();
    ...
    private  List<String> userRules(){
        ...
        String path = "/home/stream-cql-bianry-storm-2.0/bin/rule_" + taskId;
    ...
    public void initialize() throws StreamingException {
        ...
        ipFilter.setFileName("ip");
        ipFilter.setRules(userRules());
    ...
    public void execute(String streamName, TupleEvent event) throws StreamingException {
        /*
        if(stringOperator.isFilter(event, stringOperator.getRules())){
            //System.out.println(Arrays.toString(event.getAllValues()));
            for(IEmitter emitter : emitters.values()){
                emitter.emit(event.getAllValues());
            }
        }
        */
        if(ipFilter.isFilter(event, ipFilter.getRules())){
            //System.out.println(Arrays.toString(event.getAllValues()));
            for(IEmitter emitter : emitters.values()){
                emitter.emit(event.getAllValues());
            }
